...
async function call(connection_id,{withoutActiveCompanyConnection=false}={}){let handler;const requests=[],writes=[];
 ...
 ].filter(r=>!withoutActiveCompanyConnection||!['first','selected'].includes(r.id)).map(r=>({...r,environment:'sandbox',access_token:'synthetic',token_expires_at:'2099-01-01'}))};
...
test('legacy request without connection_id skips inactive connections and uses an active company connection',async()=>{
 const r=await call(undefined);
 assert.equal(r.status,200,JSON.stringify(r.body));
 assert.equal(r.requests.length,1);
 assert.ok(r.requests[0].url.includes('/company/first-realm/reports/TrialBalance'));
 assert.equal(r.requests[0].method,'GET');assert.equal(r.writes[0].connection_id,'first');
});
test('legacy request without an active company connection fails before QBO even when another company is connected',async()=>{
 const r=await call(undefined,{withoutActiveCompanyConnection:true});
 assert.equal(r.status,404);assert.equal(r.requests.length,0);assert.equal(r.writes.length,0);
});
