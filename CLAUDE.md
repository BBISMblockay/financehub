...
sample-notify    -- PUBLIC (verify_jwt off): the edge function itself still handles all five event types (SAMPLE_REQUESTED / SAMPLE_RECEIVED / SAMPLE_WAREHOUSE_READY / SAMPLE_SIZE_REQUEST / SAMPLE_ASSIGNED), but as of 20260820213000 the trg_sample_notify DB trigger only ever sends two of them: the initial insert (SAMPLE_REQUESTED or SAMPLE_RECEIVED, whichever the starting sample_status implies) and SAMPLE_SIZE_REQUEST when a Catalog-sourced pull's size_requests is set/changed. Blake/Chris in #silo-samples, 2026-08-20: every status/assignment/warehouse-ready change auto-Slacking was "managing the status rather than managing the sample" — cut down to "here is the sample with picture, manage the rest in SILO." SAMPLE_WAREHOUSE_READY and auto SAMPLE_ASSIGNED no longer fire from the trigger at all; the sample drawer's manual "Notify now" button still invokes the function directly with SAMPLE_ASSIGNED on demand (same public invoke, unaffected by the trigger change) when a specific person needs a re-ping. When product_samples.assigned_to is set, emails + Slack-DMs that one person directly (DM via SLACK_BOT_TOKEN — users.lookupByEmail on their SILO email, then conversations.open + chat.postMessage; silently skipped if the token is unset or their email isn't in the Slack workspace); otherwise falls back to emailing the active company's logistics-department members (via RESEND_API_KEY). Also posts to the separate #samples-channel webhook if SLACK_SAMPLES_WEBHOOK_URL is set (except SAMPLE_ASSIGNED, kept DM+email only) — both Slack paths are optional and skipped silently if unset, same as email without RESEND_API_KEY. Phrases the requested/size-request message differently when product_samples.request_source = 'catalog_photo_request' (names the requester, calls out "photo samples from bulk/on-hand inventory") vs a plain pipeline sample
