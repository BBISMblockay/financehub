...
| `sales-freshness-check.yml` | 11:00 / 17:00 / 23:00 UTC | Independent alarm for "the nightly sync did not run". Queries the newest `sales_by_day.day_date` per company that has an enabled `shopify_connections` row, and FAILS the run if it is more than `MAX_LAG_DAYS` (default 1) behind the last complete **Pacific** day. Reporting is the exit code — a failed Actions run emails the repo owner, so there is no extra secret or webhook to configure or let rot. **It is deliberately a separate workflow, not a step in `shopify-sync.yml`**: a check inside the sync cannot detect a dropped sync, because a dropped sync never reaches the check. That is exactly what happened 2026-08-27. Lag 1 is HEALTHY (sales are only ever complete through yesterday); 2 means a nightly was missed |
