...
`review-editor`, `review-templates`, `reviews`, `sales-verification`, `seo-overview`, `silo-chat`, `tasks`.
...
silo_business_today() / silo_business_yesterday()  -- The business runs on PACIFIC; `current_date` is UTC and is a day ahead from 17:00 Pacific onward. Any report anchoring a period on `current_date - 1` therefore headlines a PARTIAL day as "yesterday" for the last seven hours of every working day -- measured 2026-09-03 00:06 UTC: current_date-1 gave 2026-09-02 (still selling) where Pacific yesterday was 2026-09-01. It also opens a reconciliation gap, since the UTC window reaches into days the sales matview has not covered ($10,220 on YTD; zero once anchored correctly). Use these helpers, never `current_date`, for anything a person reads as a day. Pacific is hardcoded -- a tenant in another timezone needs this to read from their company record
