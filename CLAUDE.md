...
- **Dashboards** (`/v3/dashboards.html`, `/v3/dashboard.html`) — the v3 dashboard runtime: saved Ask
  ...
  second app. **Deliberately NOT in the nav while in testing** — the row exists in `nav-config.js`
  but is commented out (same stance as returns-overview); the pages work for anyone with the URL and
  ...
  **Tests live in `v3/tests/` and run in CI** (`.github/workflows/v3-tests.yml`, no secrets): `node v3/tests/run.js --unit` needs nothing installed; the browser suites need `cd v3/tests && npm install && npx playwright install chromium` and are SKIPPED (not failed) without them. Seventeen suites. Read `v3/README.md` before changing any of it — it records what was deliberately left out
