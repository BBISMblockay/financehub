...
a two-cycle budget, not an approval), evaluate every finding against the code, push ONE correction
batch per cycle, and end with a readiness assessment that names the head sha, the last reviewed
sha, and one of three statuses. The harness reads that file automatically before acting on any
