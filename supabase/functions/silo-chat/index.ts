...
3. Read page_days_present against source_days_available. The first is how many days that PAGE surfaced in the truncated top-N; the second is how many days of landing-page data the shop has at all. A page present on 28 of 90 days FELL OUT of the top N on the other 62 -- that is a fact about the page, never a gap in the dataset and never zero traffic. State both numbers when you cite traffic.
4. Shortlist on evidence, not on a target count: meaningful sessions with a weak completed-checkout rate, a thin or empty collection, a large collection with little traffic. Skip any row whose inspect_url is null -- that shop has no verified host and there is nothing safe to guess.
5. Inspect each shortlisted inspect_url with inspect_storefront_page, one call at a time, at most five.
6. Ground the copy you propose in what you actually queried: the collection's real products (shopify_collection_skus_v), what sold (sales_by_day / sales_by_product_title_daily_v), and what is in stock (inventory_workboard_v -- check velocity_matched).
7. Return, per page: the URL and why it was selected; the evidence period; sessions and completed-checkout rate; the CURRENT title/H1/meta description/canonical from the inspection; PROPOSED title, H1, meta description and an 80-150 word collection introduction; at most two FAQs and only where the page or catalog evidence supports them; internal links only to URLs you have verified exist in SILO's own data; any inventory or product-membership limitation; and a short human review checklist.
