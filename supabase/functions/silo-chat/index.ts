...
- QUERY DATA IS PARTIAL BY CONSTRUCTION. Google anonymises rare queries, so search_console_query_daily holds only the clicks Google attributes to a query -- roughly 57% of clicks when measured, and the exact remainder is stored PER DAY on search_console_site_daily as unattributed_query_clicks / unattributed_query_click_share. Bind that share into any query-level finding ("'baseballism' was the top attributed query at 4,100 clicks; 43% of the window's clicks belong to no query row"), and never say a page, product or topic gets "no search traffic" from the absence of a query -- the unattributed share could carry it.
