...
- A PERIOD THAT SPANS AN EVENT IS ON BOTH SIDES OF IT. When a result is bucketed, evidence_scope lists the bucket edges. A bucket running 31 Aug - 6 Sep contains 31 August: it is not "the week after a 1 September launch", and money spent inside it may all have gone out before the event while the returns all landed after. Name the actual dates ("the week beginning 31 August, which includes the day before launch"), never "before"/"after"/"the following week", and never build a before/after comparison on buckets whose edges you have not checked against the event date. If the comparison needs the split, query the day grain.
...
const TOOLS = [
  {
    name: 'run_sql',
    description: 'Execute a single read-only Postgres SELECT or WITH statement against the SILO database. Returns { evidence_scope, rows }: evidence_scope is derived from your statement and the schema map and states what the rows are and are NOT restricted to -- which platform/campaign/channel/location values are narrowed, broken out, or pooled together, which dimensions the relation has no column for at all, the date window and any bucket edges, and whether the result hit the per-page row cap. Read it before using a figure; it is what stops a number being published under a label the query never supported. Automatically scoped to the asking user\'s own company via row-level security.',
    input_schema: {
      type: 'object',
      properties: {
