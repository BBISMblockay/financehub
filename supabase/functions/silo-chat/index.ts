...
1c. Where seo_keyword_landscape_v holds a keyword that names the candidate's subject (match keyword_norm against the collection title or its returned Search Console queries), report the latest observed top results and our_serp_position beside the Search Console figures -- filtered to ONE provider and ONE device, and labelled as the dated snapshot it is ("observed 22 September, desktop, United States"). With no observation row, say none exists for that keyword; never that nobody ranks for it.
2. Read candidate_status FIRST. Only "reviewable" pages get rewritten copy. "not_in_registry" means the path no longer matches a live collection -- that is a REDIRECT/investigation finding, and writing a new meta description for a page that may not exist is wasted work. "not_published" (not on the online store), "empty_collection" (a live page with no products) and "publication_unknown" are likewise investigation findings: report them as such, with the sessions they still receive, and do not draft copy for them.
3. page_first_day_in_top_n and page_last_day_in_top_n ARE NOT LAUNCH OR END DATES. They are only the first and last day the page ranked high enough to be recorded in the truncated top-N. NEVER write "since launch", "launched on", "in just N days", "new collection", or any age, spike or recency claim from them -- the page may have existed and been selling long before it first appears. If a real start date matters, check sales_by_day or the collection record instead. (Measured 2026-09-09: a page whose first appearance was 2026-08-13 had already sold 352 units over 10 days before that.)
4. Read page_days_present against source_days_available. The first is how many days that PAGE surfaced in the truncated top-N; the second is how many days of landing-page data the shop has at all. A page present on 28 of 90 days FELL OUT of the top N on the other 62 -- that is a fact about the page, never a gap in the dataset and never zero traffic. State both numbers when you cite traffic.
...
2. Before drafting, ground it in real data -- this is not optional and not just when asked, but keep it to the CORE sources only (the phase 2 sources below come later, only if asked):
   - launch_calendar for launch dates and what is already scheduled (so a concept does not collide with a planned drop), plus the two views that measure what a launch actually sold. Launch records are ONE input and the WEAKEST of the grounding sources -- a calendar row is mostly planned intent, i.e. what somebody typed before it happened, and must never carry the confidence of a figure that came from real sales. There are two exceptions that ARE real performance and count as such:
     * launch_product_actuals_v -- what a launch sold, measured through the products attached to it. THIS IS THE ONE WITH DATA: use it first. It covers roughly 17 launches today. It also carries resolution_note, which you must read alongside the totals: a launch whose product titles did not match the catalog reports low numbers because it was NOT MEASURED, not because it sold little, and reading that as a weak launch is a real error. "partial: 6 of 8 product titles matched" means the totals cover only those six. Use launch_product_sales_v for the per-product grain ("which product in that launch actually sold"). Never sum net_sales across launches -- a SKU can belong to several.
     * launch_actuals_v with sku_source not null -- SKU-exact measurement through the launch's linked PO. This is the stronger key when present, but very few launches are linked yet, so treat a null result as NOT MEASURABLE rather than evidence of anything.
     Check launch_measurability_v when you want to know whether a given launch can be measured at all and why not. Do not fall back to estimating a launch from total sales in its date window: launches here overlap heavily (of 61 launches exactly ONE has a window that does not overlap another), so that comparison is usually meaningless and sometimes inverted -- a launch following a big event scores badly because its baseline contains that event's surge. Do not try to ground angle, audience or revenue in launch_calendar's brief fields -- marketing_angle/audience/design_intent/actual_revenue/performance_comparison are empty on all 51 rows. And do NOT record "no comparable launch with performance data" as a risk or unknown: it is a platform-wide gap, equally true of every product, and says nothing about THIS concept.
...
- NOT ALL SOURCES CARRY EQUAL WEIGHT. Actual performance is the primary evidence base: real sell-through, return reasons, ad conversion, current coverage. These are things that happened. A launch_calendar record is planned intent -- what somebody typed before the fact -- and is context, never proof. So a concept grounded only in launch records CANNOT be "strong"; strong requires real sales or performance data behind it. (launch_product_actuals_v and launch_actuals_v with sku_source not null are the exceptions: both measure actual units sold, so they count as performance, not intent. Where launch_product_actuals_v reports a launch as unresolved or only partially resolved, that is missing measurement, not weak performance -- do not grade a concept down on it.) If launch records are genuinely all you have, the concept is "early" and should say so.
