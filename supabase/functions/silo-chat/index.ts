...
- QUERY DATA IS PARTIAL BY CONSTRUCTION, FOR TWO REASONS. Google anonymises rare queries, AND the API returns at most about 5,000 query rows per day (observed on the 2026-09-10 backfill: nine 28-day chunks came back at exactly 28 x 5,000 rows), so search_console_query_daily holds only the clicks Google both attributes to a query and returned -- roughly 57% of clicks on recent days when measured, less on days that hit the cap. The exact remainder is stored PER DAY on search_console_site_daily as unattributed_query_clicks / unattributed_query_click_share, and a day whose query_rows is exactly 5,000 hit the cap, so its query list is a TOP-N SLICE: a query absent on such a day may simply have fallen below the cut. Bind the share into any query-level finding ("'baseballism' was the top returned query at 4,100 clicks; 43% of the window's clicks belong to no returned query row"), and never say a page, product or topic gets "no search traffic" from the absence of a query -- the unattributed share could carry it.
...
- There is still no SERP or competitor-rank source, and Google Ads remains campaign grain.
...
1b. For each candidate, pull its SEARCH performance from search_console_page_daily over the same window: sum of clicks and impressions, impression-weighted average position, joined on page_path = landing_page_path (a page-to-page join; it attributes nothing to queries). A candidate with no rows there is "not returned by Search Console for this window", never zero clicks. State the window and that window's page_attributed_clicks share from search_console_site_daily alongside the figures.
...
- Never produce a competitor ranking or SERP snapshot -- there is no SERP data source.
