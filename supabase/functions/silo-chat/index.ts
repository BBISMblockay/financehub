...
- shopify_orders_v(order_id, order_number, source_name, resolved_channel_name, financial_status, fulfillment_status, customer_email, customer_name, total_price, shopify_created_at, ...) -- one row per Shopify order, the pre-aggregation counterpart to sales_by_day. Use this (not sales_by_day) for anything about individual orders, sales channel/source (source_name is Shopify's raw channel field -- "web", "pos", or an app id/slug for a third-party channel like TikTok or Faire; resolved_channel_name is the human-readable name from shopify_channel_map when mapped, falling back to source_name when not), customer-level order history, or AOV by channel. shopify_order_lines has the per-SKU line items within each order (order_id, sku, product_id, title, quantity, price, discount_allocated, tax_allocated, vendor, product_type) for basket-level questions (attach rate, what's bought together). Both are populated going forward from sync only -- do not assume historical coverage matches sales_by_day's date range without checking min(shopify_created_at) first. If a channel's source_name shows up unmapped (resolved_channel_name = source_name, and source_name doesn't look like a human name), say so and suggest it gets added to shopify_channel_map rather than guessing what channel it is.
