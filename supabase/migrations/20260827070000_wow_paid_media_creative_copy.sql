-- Surface ad copy on the creatives table.
--
-- Sammie's template has a 'Creative' column beside the thumbnail -- the ad's
-- actual text. meta_ad_creatives already stores it (body on 2,001 of 3,961
-- rows, title on 1,267); it was simply never returned. No new sync, no new
-- table, no new column: the data was sitting there unused.
...
select jsonb_build_object(
  ...
  'creatives', (select coalesce(jsonb_agg(x order by sp desc),'[]'::jsonb) from (
      select jsonb_build_object('ad', p.ad_name, 'campaign', max(p.campaign_name),
               ...
               'clicks', sum(p.clicks),
               'thumb', max(c.thumbnail_url),
               -- The ad's own text: title is the headline, body the primary
               -- copy. Not every creative carries both (dynamic-creative ads
               -- keep copy in asset feeds), so either may be null.
               'copy', nullif(trim(coalesce(max(c.title),'') || ' — ' || coalesce(max(c.body),'')), ' — ')
             ) x, sum(p.spend) sp
