...
declare
  ...
  v_follow date;
begin
  v_tz := public.silo_company_timezone(new.company_entity_id);
...
begin
  ...
  if new.window_kind = 'baseline' then
    if public.seo_baseline_conflicts(new.period_end, v_first, v_tz) then
      raise exception
    ...
    if new.period_start <= (v_last at time zone v_tz)::date then
      raise exception
...
as $$
with pub as (
  select (max(p.published_at) at time zone public.silo_business_timezone())::date as published_on
  from public.seo_task_publications p
