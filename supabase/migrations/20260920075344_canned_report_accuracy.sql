...
set title = 'Daily Sales',
    ...
    queries_run = ARRAY[$report$with sales as materialized (
...
set title = 'Top Products — last 30 days',
    ...
    queries_run = ARRAY[$report$select product_title,
...
set title = 'Sales by Location — last 30 days',
    ...
    queries_run = ARRAY[$report$select coalesce(location_tag,'Unknown') location_tag,
...
set title = 'Ownership · Sales vs last year',
    ...
    queries_run = ARRAY[$report$with asof as (select {{as_of}} as d),
...
set title = 'Ownership · Net sales by day',
    ...
    queries_run = ARRAY[$report$select day_date,round(sum(total_net_sales),2) net_sales,sum(total_quantity_sold) units
...
set title = 'Ownership · Sales by channel',
    ...
    queries_run = ARRAY[$report$select resolved_channel_name channel,count(*) orders,
...
set title = 'Ownership · Paid media by platform',
    ...
    queries_run = ARRAY[$report$select platform,round(sum(spend),2) spend,
...
set title = 'Ownership · What the ad platforms claim vs what actually sold',
    ...
    queries_run = ARRAY[$report$with spend as materialized (
...
set title = 'Logistics · Inventory and on order',
    ...
    queries_run = ARRAY[$report$select sum(units_on_hand) units_on_hand,sum(units_on_order) units_on_order,
...
set title = 'Logistics · Cover and momentum by product type',
    ...
    queries_run = ARRAY[$report$with coverage as (
...
set title = 'Logistics · Stock with no recent sales',
    ...
    queries_run = ARRAY[$report$select product_title,product_type,sum(total_available_quantity) on_hand,
...
set title = 'Logistics · Sales relative to current stock',
    ...
    queries_run = ARRAY[$report$select product_type,
