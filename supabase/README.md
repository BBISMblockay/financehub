...
## Company document identity — `20260922130000_company_document_identity.sql`

`/v2/po-builder.html`'s vendor PDF printed `Baseballism Inc.`, a Beaverton
street address and a named buyer with a phone number as literals in the page,
so a PO generated by any tenant reached its factory under Baseballism's name
and ship-to. Nothing per-company held those facts. This migration adds them to
`company_settings` — `legal_name`, `address_line1/2`, `city`, `region`,
`postal_code`, `country`, `phone`, `purchasing_contact_name/phone/email` — all
nullable, no policy change (members read, owner_admin writes, as before). The
PDF resolves them through `v2/po-document-identity.js`: no legal name falls
back to the company title, no contact to whoever generates the PO, and no
address is **named as not configured** (in a confirm before the PDF opens and
in the document itself), never printed as another tenant's. Edited on
Workspace Settings → Company. Baseballism is seeded with exactly what its PDF
printed before, written only where null, so a settings edit is never undone by
a re-run. Ends with `refresh_chat_schema_catalog()`.
...
- `130100_business_timezone_seo` — `seo_task_publications.business_timezone`
  is stamped at insert and frozen, and the ordering triggers and
  `seo_follow_up_window` judge each publication's day in the timezone it was
  RECORDED in, so a later timezone change cannot reclassify it.
  `seo_baseline_conflicts` takes the timezone as an argument (still IMMUTABLE)
  and the Pacific-only two-argument form is dropped.
...
- `130400_business_timezone_onboarding` — LAST on purpose, and it REFUSES to
  run if any site 130000-130300 fixed still names Pacific (130300 likewise fails
  if the calendar rewrite did not take). Adds Mountain,
