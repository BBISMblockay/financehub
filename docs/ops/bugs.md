...
| **The real general ledger carries a `Not Specified` section that the archive rejects by design.** QBO groups lines that have no account under a header named `Not Specified` with no account id; the stored 366-day Baseballism report has one (24 lines: 12 Payment, 12 Journal Entry, half with blank amounts, all with transaction ids, none with a split account), and the one-month July report has one with 2 lines. `archive_qbo_ledger` refuses a headerless-or-unidentified section holding direct data rows (`Unsupported or duplicate ledger account section; raw report remains available`) because it never guesses an account from a name, and `qbo_history_lines.qbo_account_id` is `not null`. After `20260914220000` this is the next message a real import hits (unchanged by the bounded archive, `20260915000000`, which refuses it in the first call before any row is staged). Resolving it is a design decision (retain the lines under a reserved pseudo-account, or keep refusing and record the section as a coverage exception), not a parsing fix, and was deliberately left out of the number-format PR | None yet; the raw report stays in `quickbooks_report_runs` for inspection |
