...
| Issue | Workaround |
|-------|------------|
| **Ask SILO's prompt rules are asserted to REACH the prompt; nothing proves the model follows them.** The 2026-09-16 language contract (lead with the business answer, no backend vocabulary, no invented capabilities, a metric must be valid and not merely runnable) is pinned by `prompt.test.mjs`, and that suite can only ever prove the text is in the block an ordinary question is built from. Whether answers actually read differently needs eval runs against the deployed function, scored against independently verified expected results — which does not exist yet. The 30 days of `silo_chat_audit_log` reviewed for the audit are a good starting corpus, but each case still needs a verified expected answer, an explicit date/scope definition, and a rule for when SILO should DECLINE a calculation | Read new answers against the contract by hand for now; treat a green prompt suite as "the instruction is present", never as "the behaviour changed" |
| **The read-only query boundary stops writes, not arbitrary function calls.** `chat_run_readonly_query` now runs in a read-only transaction (`20260916120000`), which the executor enforces however a write is reached. It does not allowlist WHICH functions a report may call, and it would not stop a function writing outside transactional visibility (dblink, an untrusted PL opening its own connection). No such function is reachable from `authenticated` today. An allowlist is the stronger boundary and was deliberately not attempted, since it would have to enumerate every function every legitimate report already calls | Nothing needed today; re-examine if any extension or PL that writes out-of-transaction is ever granted to `authenticated` |
| **A saved Ask SILO report stores the whole tool transcript as its SQL, not the query that produced the answer.** `queries_run` is every statement the model ran that turn — schema probes, failed attempts, intermediate cuts — and the "Saved reports" modal re-runs them. `/v3/` already works around it (`defaultQueryIndex()` skips probe queries), but the saved row itself still conflates discovery with the final definition, and re-running regenerates ROWS while the saved prose explanation stays frozen at its original numbers | Check which query a saved report actually points at before building a tile on it; re-read the answer text against fresh rows rather than trusting it |
| **Sharing a saved Ask SILO answer as company-visible copies FACTS out from under the RLS that produced them.** The stored `answer` is prose. RLS scoped the query that generated it to the creator, but nothing re-filters the text on read, so an answer a manager could legitimately produce (a comp figure, a salary aggregate) becomes readable by every active company member once saved with `visibility = 'company'`. Re-running the SQL is safe — `chat_run_readonly_query` is SECURITY INVOKER — but the prose beside it is not re-derived. Policy decision pending: which result classes may be shared at all | Save anything touching compensation, payroll or an individual's numbers as `private`; the save dialog asks every time |
| **Nothing bounds Ask SILO per user or per company, and every turn resubmits the whole conversation.** No request quota, no rate limit, no end-to-end cancellation (the browser aborts its fetch; the edge function keeps running to completion and still pays for its model rounds), and no idempotency beyond the new `request_id`, which identifies a request but does not deduplicate one. Fine at ~29 people; none of it has been load-tested, so no scale claim should be made from the current behaviour | Keep an eye on `silo_chat_health_v` and the Anthropic bill; bound these before opening Ask SILO to a materially larger population |
| **`wow_creatives` was deployed ahead of this repo and no migration held the difference.** Found 2026-09-15 while adding ad destinations. `20260901170000` is the only create-or-replace for it here, and the version running in production additionally computed `thruplays`, `leads`, `cost_per_thruplay` and `cost_per_lead` at both the group and the ad grain -- which `/v2/wow-report.html` reads BY NAME (`g.cost_per_lead`, `g.cost_per_thruplay`, `a.thruplays`). The deployed body also had every explanatory comment stripped, so it was applied by hand from an edited copy rather than from `supabase/migrations/`. Rebuilding the function from the repo file would have silently deleted the Thruplays and Subscribers headlines with nothing in the diff to show it. **Reconciled by `20260915150000`**, whose body is reconstructed from `pg_get_functiondef()` against production with this repo's comments restored; `verify_v2_schema.sql` now fails CRITICAL if `cost_per_thruplay` disappears from the deployed function again. Same family as `notify_slack_sample_created()` and `notify_sample_events()`: functions live in the database, and the migration directory is not proof of what is running | Reconciled on merge + apply. When touching any RPC, read `pg_get_functiondef()` first rather than the newest migration that mentions it |
...
| **Every Beacon token is declared only inside `:root[data-theme="light"]` and `:root[data-theme="dark"]`, never on bare `:root`.** With no `data-theme` attribute on `<html>`, every `var(--bcn-*)` without a fallback is invalid at computed-value time and the page renders essentially unstyled — borders vanish, backgrounds go transparent. In practice this never happens: `v2/silo-chrome.js` sets the attribute unconditionally on mount and its toggle only ever swaps `light`/`dark`, so nothing removes it. It is recorded because it makes the whole design system depend on one line of JS running, and because a page that renders before chrome mounts, or any future "follow the system theme" option (which is normally expressed by leaving the attribute OFF), lands straight in it. Found 2026-09-15 while testing the migration flow, where a test that removed the attribute produced a convincingly page-shaped failure | Duplicate the light palette onto bare `:root` in `beacon.css` and keep the two `[data-theme]` blocks as overrides — additive, changes nothing for either current theme. Not done here: `beacon.css` is shared by every page and is not a UI-pass edit |
...
| **Two suites under `scripts/tests/` are never run by CI.** `customer-readiness-ui.test.mjs` and `seo-overview.test.mjs` are named by no workflow step, so a change breaking either goes green. `seo-overview` passes on a bare checkout as of 2026-09-22; **`customer-readiness-ui` FAILS** (`node --test scripts/tests/customer-readiness-ui.test.mjs`: "customer-facing settings do not link to implementation docs or expose secret names" -- `v2/settings-notifications.html` names an edge-function secret since its 2026-09-20 rewrite, and the unwired suite never said so). An earlier version of this line said both pass; that was read off a `tail` exit code, not the test's. Found while wiring `legacy-slack-pause.test.mjs`, which PR #750 had shipped the same way; `workflow-env-contract.test.mjs` now fails on any NEW unwired suite and pins these two in `KNOWN_UNWIRED`, so wiring one up means removing it from that set | Open — add a step to `sync-tests.yml`'s Unit job |
