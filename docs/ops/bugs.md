...
| **Two suites under `scripts/tests/` are never run by CI.** `customer-readiness-ui.test.mjs` and `seo-overview.test.mjs` are named by no workflow step, so a change breaking either goes green. Both pass on a bare checkout as of 2026-09-22 (`node --test scripts/tests/customer-readiness-ui.test.mjs`, `node scripts/tests/seo-overview.test.mjs`). Found while wiring `legacy-slack-pause.test.mjs`, which PR #750 had shipped the same way; `workflow-env-contract.test.mjs` now fails on any NEW unwired suite and pins these two in `KNOWN_UNWIRED`, so wiring one up means removing it from that set | Open — add a step to `sync-tests.yml`'s Unit job |
