/* Assertions for the v2 suites.
 *
 * Deliberately tiny and dependency-free. The unit suites must run with
 * nothing installed -- `node v2/tests/run.js --unit` on a clean checkout --
...
function createReporter(suiteName) {
  ...
  function ok(name, cond, detail) {
    if (cond) {
      pass += 1;
      if (process.env.V2_TEST_VERBOSE) console.log('  ok   ' + name);
    } else {
      fail += 1;
      failures.push(name);
  ...
  function test(name, fn) {
    try { fn(); pass += 1; if (process.env.V2_TEST_VERBOSE) console.log('  ok   ' + name); }
    catch (err) {
      fail += 1;
      failures.push(name);
