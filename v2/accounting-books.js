...
(function () {
  ...
  async function boot(){
    ...
    const surface=String(location.hash||'').slice(1),hashTab=/^[a-z]+$/.test(surface)?document.querySelector(`[data-surface="${surface}"]`):null;
    hashTab?.click();hashTab?.scrollIntoView?.({block:'start'});
    ...
    window.SiloMigrationStatus?.mount({db,companyId:company.id}).catch(e=>{console.warn('Migration status unavailable:',e.message);}).finally(()=>hashTab?.scrollIntoView?.({block:'start'}));
