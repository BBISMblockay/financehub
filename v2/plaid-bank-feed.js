...
(function () {
  ...
  function eligibleForAi(transaction, source) {
    return isAvailable(transaction) && Number(transaction.amount) > 0
  ...
  function create({ db, company, references, dirty, changed, openBatch, status, document: doc = document,
    ...
    let connections = [], accounts = [], exceptions = [], busy = false, linkHandler = null;
    ...
    async function invoke(action, values = {}) {
      ...
      if (error || data?.error) throw new Error(data?.error || error?.message || 'Bank feed request failed.');
    ...
    async function load() {
      ...
      const results = await Promise.all([
        db.from('plaid_connections').select('id,institution_name,environment,status,last_error_code').eq('company_entity_id', current.companyId).order('created_at'),
        db.from('plaid_accounts').select('id,connection_id,name,mask,type,subtype,iso_currency_code,current_balance,available_balance,balance_updated_at,source_id,last_synced_at,last_error_code').eq('company_entity_id', current.companyId).order('name'),
    ...
    function render() {
      ...
      byId('bankAccounts').innerHTML = accounts.map((account) => {
        ...
        return `<div class="cc-card" data-bank-account="${esc(account.id)}"><div class="cc-card-head"><div>
          ...
          <span class="cc-chip">${source ? 'Feeds coding' : 'Not mapped'}</span></div><div class="cc-pad">
          ...
          <div class="cc-grid" style="margin-top:12px">
            <label class="cc-field">Coding source<select class="bcn-field" data-bank-source ${disabled ? 'disabled' : ''}><option value="">Create a source for this account</option>${sourceOptions}</select></label>
          ...
          </div><div class="cc-note" style="margin-top:8px">CSV may cover dates before the cutover only. Purchases already recorded elsewhere must not be imported again.</div>
          </div><div class="cc-foot">
          ${!source ? `<button class="bcn-btn" data-bank-map ${disabled ? 'disabled' : ''}>Save account mapping</button>` : ''}
          <button class="bcn-btn bcn-btn--primary" data-bank-sync ${!source || connection?.status !== 'active' ? 'disabled' : ''}>Sync transactions</button>
          <span class="cc-note">${source ? `Uses ${esc(source.display_name)}. Review and approve in Coding.` : 'Mapping does not enable QBO posting.'}</span></div></div>`;
      }).join('');
    ...
    byId('bankAccounts').addEventListener('click', (event) => {
      ...
      if (event.target.closest('[data-bank-sync]')) return run(async () => {
        const result = await invoke('sync', { account_id: accountId });
        if (!Array.isArray(result.batch_ids) || !Number.isFinite(Number(result.exceptions))) throw new Error('Sync result could not be confirmed. Refresh bank feeds before retrying.');
        await changed(); await load();
        status(`Sync finished. ${Number(result.exceptions || 0)} change(s) need review. Open an imported batch to code it.`, 'pos');
